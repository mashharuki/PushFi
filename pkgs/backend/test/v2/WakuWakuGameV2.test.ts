...
describe("WakuWakuGameV2", function () {
  ...
  async function deployContract() {
    ...
    const superNft: WakuWakuSuperNFT = await WakuWakuNFT.deploy(
      await owner.getAddress()
    );
    // deploy game contract
    const WakuWakuGameV2: WakuWakuGameV2__factory =
      await ethers.getContractFactory("WakuWakuGameV2");
    const game: WakuWakuGameV2 = await WakuWakuGameV2.deploy(
      await owner.getAddress()
    );
  ...
  describe("Game", function () {
    ...
    it("play game test 【 10 times】", async function () {
      // deploy contract
      const {owner, otherAccount, superNft, nft, game} = await loadFixture(
        deployContract
      );
      // create new game
      await createNewGame(game, superNft.address, nft.address);
      ...
      const superNftBalance2 = await superNft.balanceOf(
    ...
    it("play game test 【 20 times】", async function () {
      // deploy contract
      const {owner, otherAccount, otherAccount2, superNft, nft, game} =
      ...
      const superNftBalance3 = await superNft.balanceOf(
